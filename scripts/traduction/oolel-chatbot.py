...
DEFAULT_MAX_NEW_TOKENS = 200 # Valeur par défaut de max_new_tokens
DEFAULT_TEMPERATURE = 1 # Valeur par défaut de temperature
...
class TranslationService:
    """Handles translation and evaluation using an API."""
    def __init__(self, max_new_tokens: int = DEFAULT_MAX_NEW_TOKENS, temperature: float = DEFAULT_TEMPERATURE):
         """Initializes the service with the specified model."""
         load_dotenv()
         self.client = Client("dofbi/Wolof-Bridge") # Or Client("dofbi/Wolof-Bridge", hf_token=HF_TOKEN)
         self.system_prompt = (
    ...
    def translate_and_evaluate(self, wolof_phrase: str, reference_translation: str,
        ...
        """
        Translates a wolof phrase using the API and evaluates the translation.
        Args:
...
def main():
    ...
    # Définir les valeurs personnalisées pour max_new_tokens et temperature
    max_new_tokens_value = 200
    temperature_value = 0.9

    translation_service = TranslationService(max_new_tokens=max_new_tokens_value, temperature=temperature_value)
